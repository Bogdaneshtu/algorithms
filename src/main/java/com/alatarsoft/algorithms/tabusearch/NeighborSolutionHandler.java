...
public class NeighborSolutionHandler {
    ...
    /**
     * In every iteration we have to found the next state: so we have to consider the
    ...
    public State getBestNeighbor(State[][] states, List<State> neighborStates, List<State> tabuStates) {
        // it means we want to remove all items present in tabuStates from neighborStates list
